...
| 内存 | 125GB |
| 存储 | `/mnt/nvfile` - nvfile 自研高性能存储集群 (113TB) |
...
python /mnt/data/vllmtest/LMCache/benchmarks/long_doc_qa/long_doc_qa.py \
  ...
  --json-output \
  2>&1 | tee /mnt/data/vllmtest/results/cascade_long_doc_10k.log
```

### Cascade 自定义 Benchmark

```bash
python3 /mnt/data/vllmtest/scripts/cascade_bench.py \
  --host 127.0.0.1 --port 8000 \
  --model qwen25-7b \
  --num-docs 10 --doc-len 4096 \
  --max-tokens 10 --concurrent 2 \
  --output /mnt/data/vllmtest/results/bench_posix.json
```
